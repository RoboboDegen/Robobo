...
module robobo::calculate_tests {
    ...
    fun create_defense_hash(): vector<u8> {
        let mut hash = vector::empty<u8>();
        let mut i = 0;
        while (i < 32) {
            vector::push_back(&mut hash, 7u8);
            i = i + 1;
        };
        hash
    }
    ...
    fun test_basic_battle() {
        ...
        assert!(final_defender_energy == 131, 1);
       
        assert!(winner == false, 2); // 防守者获胜，因为会触发回血技能
    }
    ...
    fun test_battle_round_limit() {
        ...
        assert!(final_attacker_energy == 128, 0);
        assert!(final_defender_energy == 133, 1);
    }
    ...
    fun test_personality_defense_style() {
        ...
        assert!(final_attacker_energy == 128, 0);
        assert!(final_defender_energy == 142, 1);
    }
    ...
    fun test_high_attack_vs_high_defense() {
        ...
        assert!(final_attacker_energy == 134, 0);
        assert!(final_defender_energy == 128, 1);
    ...
    fun test_speed_personality_combination() {
        ...
        //128
        // debug::print(&final_defender_energy);
        assert!(final_attacker_energy == 142, 0);
        assert!(final_defender_energy == 128, 1);
    }
    ...
    fun test_energy_management() {
        ...
        // 激进的战斗风格应该消耗更多能量
        assert!(final_attacker_energy == 142, 2);
    ...
    fun test_consecutive_actions_with_speed() {
        ...
        assert!(final_attacker_energy == 128, 0); 
        assert!(final_defender_energy == 132, 1); 
    }
...
//     // // ========== 临界值测试 ==========
    ...
    fun test_critical_attribute_difference() {
        ...
        assert!((final_attacker_energy as u64) - (final_defender_energy as u64) <= 10, 1); 
        assert!(final_attacker_energy == 132, 2); 
        assert!(final_defender_energy == 128, 3); 
    }
    ...
    fun test_energy_efficiency() {
        ...
        assert!(final_attacker_energy == 148, 2); 
        assert!(final_defender_energy == 128, 3); 
    }
    ...
    fun test_battle_momentum() {
        ...
        assert!(final_attacker_energy == 136, 2); 
        assert!(final_defender_energy == 128, 3); 
    }
    ...
    fun test_personality_exact_damage() {
        ...
        assert!(final_attacker_energy == 132, 0);
        assert!(final_defender_energy == 128, 1);
    }
    ...
    #[test]
    fun test_both_defense_recovery() {
        // 使用全7的hash使行为可预测
        let battle_hash = create_defense_hash();
