...
module robobo::calculate_tests {
    ...
    fun test_battle_with_extreme_attribute_difference() {
        ...
        // 确保能量值在合理范围内
        assert!(final_attacker_energy >= 127 && final_attacker_energy <= 188, 2);
        assert!(final_defender_energy >= 127 && final_defender_energy <= 168, 3);
    }
    ...
    fun test_battle_round_limit() {
        ...
        // 验证战斗是否在合理回合数内结束
        assert!(final_attacker_energy >= 127 && final_attacker_energy <= 188, 0);
        assert!(final_defender_energy >= 127 && final_defender_energy <= 188, 1);
        // 战斗应该在能量耗尽或达到回合限制时结束
    ...
    fun test_near_death_battle() {
        ...
        // 验证战斗结果
        // 战斗应该很快结束
        assert!(final_attacker_energy >= 127 && final_attacker_energy < 130, 0);
        assert!(final_defender_energy >= 127 && final_defender_energy < 130, 1);
    }
